...
// Non-image binary files this pipeline can produce (trained weights, archives)
// that must never be syntax-highlighted or opened in the text editor.
...
export const FileViewDialog = React.memo<{
  ...
  const isImage = useMemo(() => {
    return fileExtension in IMAGE_MIME_MAP;
  }, [fileExtension]);

  const isBinary = useMemo(() => {
    return BINARY_EXTENSIONS.has(fileExtension);
  }, [fileExtension]);

  // Dataset/training subagents write real image files and model weights into
  // the shared workspace - these can't be safely edited as text, so preview
  // and editing both need a non-text path for them.
  const isPreviewOnly = isImage || isBinary;

  const imageDataUri = useMemo(() => {
    if (!isImage || !fileContent) return null;
    if (fileContent.startsWith("data:")) return fileContent;
    const mime = IMAGE_MIME_MAP[fileExtension] || "image/png";
                  ...
                  imageDataUri && !imageLoadError ? (
                    ...
                    <div className="flex flex-col items-center justify-center gap-2 p-12 text-center">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Preview not available for this image - use Download to
                        view it externally.
                      </p>
                    </div>
                  )
