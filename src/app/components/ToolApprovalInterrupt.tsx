...
export function ToolApprovalInterrupt({
  ...
  const isApprovalRequest = actionRequest.name === "request_approval";
  const stage = useMemo(() => {
    const value = actionRequest.args?.["stage"];
    return typeof value === "string" ? value : null;
  }, [actionRequest.args]);
  const summary = useMemo(() => {
    const value = actionRequest.args?.["summary"];
    return typeof value === "string" ? value : null;
  }, [actionRequest.args]);
  ...
  return (
    <div className="w-full rounded-md border border-border bg-muted/30 p-4">
      ...
      <div className="mb-4 rounded-sm border border-border bg-background p-3">
        ...
        ) : isApprovalRequest && summary !== null ? (
          <div>
            <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
              Summary
            </span>
